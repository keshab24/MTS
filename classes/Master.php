...
Class Master extends DBConnection {
 ...
 function save_report(){
    ...
    function dt_reports(){
		...
		extract($_POST);
		$totalCount = $this->conn->query("SELECT * FROM `report_images` where user_id = '{$this->settings->userdata('id')}' ")->num_rows;
		...
		$columns_arr = array("unix_timestamp(created_at)",
							...
							"image",
							"");
		$query = $this->conn->query("SELECT * FROM `report_images`  where user_id = '{$this->settings->userdata('id')}'  {$search_where} ORDER BY {$columns_arr[$order[0]['column']]} {$order[0]['dir']} limit {$length} offset {$start} ");
		$recordsFilterCount = $this->conn->query("SELECT * FROM `report_images`  where user_id = '{$this->settings->userdata('id')}'  {$search_where} ")->num_rows;
	...
	function delete_report(){
		extract($_POST);
		$id = $_POST['id'];
		$this->conn->query("DELETE FROM report_images where id = 7");

		// Perform necessary validation and authentication
		
		// Perform the delete operation in your database or data source
		// Replace this code with your actual delete logic
		$success = false;
		if ($id) {
		  // Perform the delete operation, e.g., delete from a database table
		  // Replace 'your_table_name' with the actual table name in your database
		  // Replace 'your_id_column' with the actual ID column name in your table
		
		$delete = $this->conn->query("DELETE FROM report_images where id = '{$id}' ");
		}
		if($delete){
			$resp['status'] = 'success';
