                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm table-hover table-stripped">
                        ...
                        <tbody>
                            ...
                            $today = date('l');
                            // var_dump($today);
                           //added query you can use the LIKE operator with wildcards to check if the current day of the week is in the list of days for a particular schedule.
                        //    so using %{$today}% will match any schedule that includes today's day of the week, regardless of its position in the list.
                            $med_sched_sql = "SELECT sl.*, ml.name as med_name, ml.description as med_description FROM `schedule_list` sl inner join medicine_list ml on ml.id = sl.medicine_id where sl.user_id = '{$_settings->userdata('id')}' and sl.`day` LIKE '%{$today}%' and date_format(`sl`.`date_start`, '%Y-%m-%d') < '{$now}' and (until IS NULL OR date_format(`sl`.`until`, '%Y-%m-%d') >= '{$now}')  order by abs(unix_timestamp(CONCAT('{$now} ',`sl`.`time`)))";
